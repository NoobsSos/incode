...
function Main() {
  ...
  const moveToDo = (id: string) => {
    ...
    task
      .updateTask(id, { ...tasks[taskIndex], status: "To do" })
      .then((res) => {
        setTasks([
  ...
  const moveInProgress = (id: string) => {
    ...
    task
      .updateTask(id, { ...tasks[taskIndex], status: "In progress" })
      .then((res) => {
        setTasks([
          ...tasks.slice(0, taskIndex),
          res.data,
    ...
    <div className={styles.wrapper}>
      <header>
        <div className={styles.header_container}>
          ...
          />
          <button
            onClick={() => createOrLoadTable(tableName)}
                      ...
                      (
                        <div key={task._id} className={styles.card}>
                          ...
                          <div className={styles.due_date}>
                            Finish to {new Date(task.due).toLocaleDateString()}
                          </div>
                          <button
                            onClick={() => moveInProgress(task._id)}
                ...
                tasks
                  ...
                  .map((task) => (
                    <div key={task._id} className={styles.card}>
                      ...
                      <button
                        onClick={() => moveClosed(task._id)}
                        className={styles.button_move}
                      >
                        Move to Closed{" "}
                      </button>
                      <button
                        onClick={() => moveToDo(task._id)}
                        className={styles.button_move}
                      >
                        Back{" "}
                      </button>
      ...
      {modal && modal === "create" && (
        <div className={styles.center}>
          <ModalCreate
            closeModal={() => dispatch(taskActions.setModal(""))}
            status={status}
          />
        </div>
      )}
